...
{
  int i;
  if(data==0||data==1)//Perhaps the data is abnormal
  {
    return;
  }	
...
{	
	...
	sync_read = adc_sync_read(ip->client);
	bpw16n_report(ip->input_dev,ip->adc_val); //OLD CODE
	//bpw16n_report(ip->input_dev,sync_read); //Display Lux on Screen - A.T. 3.0.8+
	if(ip->enabled){	
